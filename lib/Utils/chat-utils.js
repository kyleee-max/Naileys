...
/**
 * @typedef {{ version: number, hash: Buffer, indexValueMap: Record<string, { valueMac: Buffer }> }} LTState
 * @typedef {{ type: string, index: unknown, syncAction: unknown, apiVersion: number, operation: number }} SyncdPatchParams
 * @typedef {(keyId: string) => Promise<unknown>} AppStateKeyFetcher
 * @typedef {(data: unknown) => void} MutationCallback
...
/**
 * Create a fresh LT hash state used to generate anti-tampering hashes.
 * @returns {{ version: number, hash: Buffer, indexValueMap: Record<string, any> }}
 */
