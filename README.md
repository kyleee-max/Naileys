# <div align='center'>Naileys</div>

<div align='center'>

[![npm version](https://img.shields.io/npm/v/naileys.svg)](https://www.npmjs.com/package/naileys)
[![License](https://img.shields.io/badge/license-GPL%203-blue.svg)](LICENSE)
[![Downloads](https://img.shields.io/npm/dm/naileys.svg)](https://www.npmjs.com/package/naileys)
![Naileys Banner](https://files.catbox.moe/4flt15.jpg)
</div>
---
...
Pengelola Naileys tidak mendukung penggunaan aplikasi ini untuk melanggar Ketentuan Layanan WhatsApp. Kami menekankan tanggung jawab pribadi pengguna untuk menggunakan secara adil dan bertanggung jawab.
...
```bash
npm i naileys@latest
# atau
yarn add naileys@latest
```

### Impor di Kode

```javascript
const { default: makeWASocket } = require("naileys")
// atau ES6
...
```javascript
const { default: makeWASocket, DisconnectReason, useMultiFileAuthState } = require('naileys')
    ...
    sock.ev.on('messages.upsert', async ({ messages }) => {
        for (const m of messages) {
            ...
            await sock.sendMessage(m.key.remoteJid!, { 
                text: 'Halo! Saya bot WhatsApp menggunakan Naileys 🤖' 
            })
        }
    })
...
WhatsApp menyediakan API multi-device yang memungkinkan Naileys terautentikasi sebagai klien WhatsApp sekunder melalui QR code atau pairing code.
...
```javascript
const { default: makeWASocket, Browsers } = require("naileys")
...
2. Secara default, Naileys menggunakan konfigurasi Chrome. Untuk koneksi seperti desktop (untuk riwayat pesan lebih banyak), gunakan:
...
```javascript
const makeWASocket = require("naileys").default
const { useMultiFileAuthState } = require("naileys")
...
```javascript
const makeWASocket = require("naileys").default
const { DisconnectReason, useMultiFileAuthState } = require("naileys")
const { Boom } = require('@hapi/boom')
...
Naileys tidak menyertakan penyimpanan default untuk obrolan, kontak, atau pesan. Namun, implementasi in-memory sederhana disediakan. Store mendengarkan update obrolan, pesan baru, dll., untuk menjaga data tetap terkini.
...
```javascript
const makeWASocket = require("naileys").default
const { makeInMemoryStore } = require("naileys")

const store = makeInMemoryStore({ })

store.readFromFile('./naileys_store.json')

setInterval(() => {
    store.writeToFile('./naileys_store.json')
}, 10_000)
...
2. Naileys punya fungsi untuk pratinjau tautan.
...
await sock.sendMessage(
    jid,
    {
        text: 'Halo, ini dikirim menggunakan https://npmjs.com/naileys'
    }
)
```
...
Naileys tidak muat seluruh buffer ke memori; enkripsi sebagai stream.
...
import { downloadMediaMessage, getContentType } from 'naileys'
...
Naileys dirancang untuk fungsi kustom. Tambahkan ekstensi sendiri tanpa fork.

### Aktifkan Level Debug di Log Naileys
...
<div align="center">
  Fork dan dimodifikasi oleh Tim Naileys.  
  Naileys - API WhatsApp Web Modern dengan Perbaikan @lid Ke @pn
</div>
